...
func (m *InstrumentedState) doMipsStep() error {
	...
	// Fetch the current program counter (PC)
	pc := m.state.GetPC()
	// Convert PC from byte address to instruction index (since each instruction is 4 bytes)
	cacheIdx := pc / 4

	var insn, opcode, fun uint32
	if int(cacheIdx) < len(m.cached_decode) {
		// If the instruction is within the cache, use the cached decoded instruction
		insn_detail := m.cached_decode[cacheIdx]
		insn, opcode, fun = insn_detail.insn, insn_detail.opcode, insn_detail.fun
	} else {
		// Fallback: If PC is outside the cache, read and decode the instruction from memory
		raw := m.state.Memory.GetWord(pc)
		insn = uint32(raw >> 32) // Instruction is in the upper 32 bits becuase cannon uses big-endian
		opcode = (insn >> 26) & 0x3F
		fun = insn & 0x3F
	}
