	...
	for i, tt := range cases {
		for _, v := range GetMipsVersionTestCases(t) {
			...
			t.Run(testName, func(t *testing.T) {
				pc := Word(0x0)
				goVm := v.VMFactory(nil, os.Stdout, os.Stderr, testutil.CreateLogger(), mtutil.WithRandomization(int64(i)), mtutil.WithPCAndNextPC(pc), mtutil.WithRegionSize(testCodeRegionSize, testHeapSize))
				state := goVm.GetState()
				var insn uint32
				var rtReg uint32
				var rdReg uint32
