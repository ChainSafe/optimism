		...
		for _, v := range versions {
			t.Run(v.Name, func(t *testing.T) {
				goVm := v.VMFactory(nil, os.Stdout, os.Stderr, testutil.CreateLogger(),
					mtutil.WithRandomization(seed), mtutil.WithHeap(heap), mtutil.WithRegionSize(testCodeRegionSize, testHeapSize))
				state := goVm.GetState()
				step := state.GetStep()
...
func FuzzStateSyscallExitGroup(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, exitCode uint8, seed int64) {
		for _, v := range versions {
			t.Run(v.Name, func(t *testing.T) {
				goVm := v.VMFactory(nil, os.Stdout, os.Stderr, testutil.CreateLogger(),
					mtutil.WithRandomization(seed), mtutil.WithRegionSize(testCodeRegionSize, testHeapSize))
				state := goVm.GetState()
		...
		for _, v := range versions {
			t.Run(v.Name, func(t *testing.T) {
				...
				goVm := v.VMFactory(oracle, os.Stdout, os.Stderr, testutil.CreateLogger(),
					mtutil.WithRandomization(seed), mtutil.WithPreimageKey(preimageKey), mtutil.WithRegionSize(testCodeRegionSize, testHeapSize))
				state := goVm.GetState()
	...
	f.Fuzz(func(t *testing.T, addr arch.Word, pc arch.Word, count arch.Word, preimageOffset arch.Word, seed int64) {
		for _, v := range versions {
			t.Run(v.Name, func(t *testing.T) {
				...
				goVm := v.VMFactory(oracle, os.Stdout, os.Stderr, testutil.CreateLogger(),
					mtutil.WithRandomization(seed), mtutil.WithPreimageKey(preimageKey), mtutil.WithPreimageOffset(preimageOffset), mtutil.WithPCAndNextPC(pc), mtutil.WithRegionSize(testCodeRegionSize, testHeapSize))
				state := goVm.GetState()
				state.GetRegistersRef()[2] = arch.SysRead
	...
	f.Fuzz(func(t *testing.T, addr Word, count Word, hint1, hint2, hint3 []byte, randSeed int64) {
		for _, v := range versions {
			t.Run(v.Name, func(t *testing.T) {
				...
				goVm := v.VMFactory(oracle, os.Stdout, os.Stderr, testutil.CreateLogger(),
					mtutil.WithRandomization(randSeed), mtutil.WithLastHint(lastHint), mtutil.WithPCAndNextPC(pc), mtutil.WithRegionSize(testCodeRegionSize, testHeapSize))
				state := goVm.GetState()
	...
	f.Fuzz(func(t *testing.T, addr arch.Word, count arch.Word, seed int64) {
		for _, v := range versions {
			t.Run(v.Name, func(t *testing.T) {
				...
				goVm := v.VMFactory(oracle, os.Stdout, os.Stderr, testutil.CreateLogger(),
					mtutil.WithRandomization(seed), mtutil.WithPreimageKey(preimageKey), mtutil.WithPreimageOffset(128), mtutil.WithPCAndNextPC(pc), mtutil.WithRegionSize(testCodeRegionSize, testHeapSize))
				state := goVm.GetState()
				state.GetRegistersRef()[2] = arch.SysWrite
