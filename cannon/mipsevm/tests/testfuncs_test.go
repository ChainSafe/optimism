	...
	for _, v := range versions {
		for i, tt := range cases {
			...
			t.Run(testName, func(t *testing.T) {
				...
				goVm := v.VMFactory(nil, os.Stdout, os.Stderr, testutil.CreateLogger(), mtutil.WithRandomization(int64(i)), mtutil.WithPC(0), mtutil.WithNextPC(4), mtutil.WithRegionSize(testCodeRegionSize, testHeapSize))
				state := goVm.GetState()
				var insn uint32
	...
	for _, v := range GetMipsVersionTestCases(t) {
		for i, tt := range cases {
			...
			t.Run(testName, func(t *testing.T) {
				...
				goVm := v.VMFactory(nil, os.Stdout, os.Stderr, testutil.CreateLogger(), mtutil.WithRandomization(int64(i)), mtutil.WithPCAndNextPC(0), mtutil.WithRegionSize(testCodeRegionSize, testHeapSize))
				state := goVm.GetState()
...
func testBranch(t *testing.T, cases []branchTestCase) {
	...
	for _, v := range versions {
		for i, tt := range cases {
			testName := fmt.Sprintf("%v (%v)", tt.name, v.Name)
			t.Run(testName, func(t *testing.T) {
				goVm := v.VMFactory(nil, os.Stdout, os.Stderr, testutil.CreateLogger(), mtutil.WithRandomization(int64(i)), mtutil.WithPCAndNextPC(tt.pc), mtutil.WithRegionSize(testCodeRegionSize, testHeapSize))
				state := goVm.GetState()
				const rsReg = 8 // t0
		...
		for i, c := range cases {
			for _, llVariation := range llVariations {
				...
				t.Run(tName, func(t *testing.T) {
					...
					goVm := ver.VMFactory(nil, os.Stdout, os.Stderr, testutil.CreateLogger(), mtutil.WithRandomization(int64(i)), mtutil.WithPCAndNextPC(0x08), mtutil.WithRegionSize(testCodeRegionSize, testHeapSize))
		...
		for i, c := range cases {
			for _, llVariation := range llVariations {
				...
				t.Run(tName, func(t *testing.T) {
					...
					goVm := ver.VMFactory(oracle, os.Stdout, os.Stderr, testutil.CreateLogger(), mtutil.WithRandomization(int64(i)), mtutil.WithRegionSize(testCodeRegionSize, testHeapSize))
					state := mtutil.GetMtState(t, goVm)
					step := state.GetStep()
