...
//!
//! This mirrors reth's [`HistoricalStateProviderRef`] pattern (see `historical.rs`),
//! adapted for "end-of-block" semantics (value *after* `max_block_number`'s execution).
...
/// Where to read the value for a given key at a specific block.
...
/// Factory for creating v2 history-aware hashed state cursors from a database transaction.
///
/// Wraps a database transaction and produces cursors over the v2 tables
/// that resolve historical values using the 3-table lookup strategy.
#[derive(Debug, Clone)]
pub struct V2HashedCursorFactory<TX> {
    tx: TX,
    max_block_number: u64,
    /// When `true`, the target block is the latest — skip history/changeset
    /// lookups entirely and read straight from the current-state tables.
    is_latest: bool,
}

impl<TX> V2HashedCursorFactory<TX> {
    /// Create a new [`V2HashedCursorFactory`] with a target block number.
    ///
    /// `is_latest` should be `true` when `max_block_number` equals the latest
    /// block in the proof window. This enables a fast path that skips all
    /// history/changeset lookups.
    pub const fn new(tx: TX, max_block_number: u64, is_latest: bool) -> Self {
        Self { tx, max_block_number, is_latest }
    }
}

impl<TX: DbTx> HashedCursorFactory for V2HashedCursorFactory<&TX> {
    type AccountCursor<'a>
        = V2AccountCursor<
        <TX as DbTx>::Cursor<HashedAccounts>,
        <TX as DbTx>::Cursor<HashedAccountsHistory>,
        <TX as DbTx>::DupCursor<HashedAccountChangeSets>,
    >
    where
        Self: 'a;
    type StorageCursor<'a>
        = V2StorageCursor<
        <TX as DbTx>::DupCursor<HashedStorages>,
        <TX as DbTx>::Cursor<HashedStoragesHistory>,
        <TX as DbTx>::DupCursor<HashedStorageChangeSets>,
    >
    where
        Self: 'a;
...
/// Factory for creating v2 history-aware trie cursors from a database transaction.
///
/// Wraps a database transaction and produces cursors over the v2 trie tables
/// that resolve historical values using the 3-table lookup strategy.
#[derive(Debug, Clone)]
pub struct V2TrieCursorFactory<TX> {
    tx: TX,
    max_block_number: u64,
    /// When `true`, skip history/changeset lookups and read from current-state
    /// tables directly.
    is_latest: bool,
}

impl<TX> V2TrieCursorFactory<TX> {
    /// Create a new [`V2TrieCursorFactory`] with a target block number.
    ///
    /// `is_latest` should be `true` when `max_block_number` equals the latest
    /// block in the proof window.
    pub const fn new(tx: TX, max_block_number: u64, is_latest: bool) -> Self {
        Self { tx, max_block_number, is_latest }
    }
}

impl<TX: DbTx> TrieCursorFactory for V2TrieCursorFactory<&TX> {
    type AccountTrieCursor<'a>
        = V2AccountTrieCursor<
        <TX as DbTx>::Cursor<AccountsTrie>,
        <TX as DbTx>::Cursor<AccountsTrieHistory>,
        <TX as DbTx>::DupCursor<AccountTrieChangeSets>,
    >
    where
        Self: 'a;
    type StorageTrieCursor<'a>
        = V2StorageTrieCursor<
        <TX as DbTx>::DupCursor<StoragesTrie>,
        <TX as DbTx>::Cursor<StoragesTrieHistory>,
        <TX as DbTx>::DupCursor<StorageTrieChangeSets>,
    >
    where
        Self: 'a;
...
mod tests {
    ...
    #[test]
    fn hashed_cursor_factory_produces_working_cursors() {
        let db = setup_db();
        let addr = B256::from([0xAA; 32]);
        let slot = B256::from([0x01; 32]);

        {
            ...
            wtx.cursor_write::<HashedAccounts>()
                .expect("c")
                .upsert(addr, &Account { nonce: 42, ..Default::default() })
                .expect("upsert");
            wtx.cursor_dup_write::<HashedStorages>()
                .expect("c")
                .upsert(addr, &StorageEntry { key: slot, value: U256::from(99u64) })
                .expect("upsert");
            wtx.commit().expect("commit");
        }

        let tx = db.tx().expect("ro tx");
        let factory = V2HashedCursorFactory::new(&tx, u64::MAX, true);

        let mut acc_cur = factory.hashed_account_cursor().expect("ok");
        let (k, v) = acc_cur.seek(addr).expect("ok").expect("some");
        assert_eq!(k, addr);
        assert_eq!(v.nonce, 42);

        let mut sto_cur = factory.hashed_storage_cursor(addr).expect("ok");
        let (k, v) = sto_cur.seek(slot).expect("ok").expect("some");
        assert_eq!(k, slot);
        assert_eq!(v, U256::from(99u64));
    }

    #[test]
    fn trie_cursor_factory_produces_working_cursors() {
        let db = setup_db();
        let addr = B256::from([0xBB; 32]);
        let path = Nibbles::from_nibbles([0x01]);
        ...
        let tx = db.tx().expect("ro tx");
        let factory = V2TrieCursorFactory::new(&tx, u64::MAX, true);

        let mut acc_cur = factory.account_trie_cursor().expect("ok");
        let (k, _) = TrieCursor::seek(&mut acc_cur, path).expect("ok").expect("some");
        assert_eq!(k, path);

        let mut sto_cur = factory.storage_trie_cursor(addr).expect("ok");
        let (k, _) = TrieCursor::seek(&mut sto_cur, path).expect("ok").expect("some");
        assert_eq!(k, path);
    }

    #[test]
    fn hashed_cursor_factory_with_historical_block() {
        let db = setup_db();
...

        {
            ...
            // Current state: nonce=50
            wtx.cursor_write::<HashedAccounts>()
                .expect("c")
                .upsert(addr, &sample_account(50))
                .expect("upsert");
            ...
            wtx.cursor_write::<HashedAccountsHistory>()
                ...
                .upsert(
                    HashedAccountShardedKey::new(addr, u64::MAX),
                    &BlockNumberList::new_pre_sorted([20]),
                )
                .expect("upsert");

            // Changeset at 20: nonce=10
            wtx.cursor_dup_write::<HashedAccountChangeSets>()
                .expect("c")
                .append_dup(20u64, HashedAccountBeforeTx::new(addr, Some(sample_account(10))))
                .expect("append");
        ...
        // Factory at block 15 → should read from changeset (block 20, old=10)
        let factory = V2HashedCursorFactory::new(&tx, 15, false);
        let mut cur = factory.hashed_account_cursor().expect("ok");
        let (_, v) = cur.seek(addr).expect("ok").expect("some");
        assert_eq!(v.nonce, 10, "historical lookup at block 15 via factory");

        // Factory at block 25 → should read from current state (nonce=50)
        let factory2 = V2HashedCursorFactory::new(&tx, 25, false);
        let mut cur2 = factory2.hashed_account_cursor().expect("ok");
        let (_, v2) = cur2.seek(addr).expect("ok").expect("some");
        assert_eq!(v2.nonce, 50, "post-modification lookup at block 25 via factory");
    }
}
