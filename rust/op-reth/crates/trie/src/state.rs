...
impl InMemoryState {
    ...
    /// Prunes blocks from the buffer that are strictly after the given block number.
    ///
    /// This is used during chain reorganizations (unwinds) to remove invalidated blocks
    /// from the tip of the in-memory buffer.
    pub(crate) fn prune_after(&self, number: u64) {
        let mut blocks = self.blocks.write();
        let mut numbers = self.numbers.write();

        let mut to_remove = Vec::new();
        // numbers is sorted BTreeMap. Iterate backwards.
...
impl LiveTrieState {
    ...
    /// Prunes blocks from the buffer that are strictly after the given block number.
    pub fn prune_after(&self, number: u64) {
        self.inner.prune_after(number);
    }
