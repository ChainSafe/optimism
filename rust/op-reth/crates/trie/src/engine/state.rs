...
pub(crate) struct PersistenceState {
    ...
    /// Handle to the persistence service, used to trigger saves and unwinds.
    handle: PersistenceHandle,
    /// Reply channel for the in-flight save. Present only while a save is running.
    in_flight: Option<Receiver<Option<u64>>>,
}
...
impl PersistenceState {
    ...
    pub(crate) fn unwind(
        ...
        match rx.recv_timeout(Duration::from_secs(DEFAULT_PERSISTENCE_TIMEOUT_SECS)) {
            Ok(Ok(())) => Ok(()),
            Ok(Err(reason)) => Err(EngineError::UnwindFailed(reason)),
...
/// All mutable state owned by the collector engine.
///
/// By isolating state from the event-loop plumbing, each [`EngineAction`]
/// can be handled by a self-contained task struct that receives
/// `&mut EngineState` — analogous to `EngineState` in kona's
/// `EngineTaskExt` pattern.
    ...
    Store: OpProofsStore + Clone + 'static,
{
    ...
    /// Poll for completed saves, apply backpressure if the buffer is oversized,
    /// and kick off a new background save when the persistence threshold is met.
    ///
